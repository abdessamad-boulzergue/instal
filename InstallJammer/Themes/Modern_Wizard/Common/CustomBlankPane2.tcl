...
proc CreateWindow.CustomBlankPane2 { wizard id } {
    ...
    set id2 "3985BA13-13BB-46E8-8571-474B6D77B2DB"
    set ver [::InstallAPI::SubstVirtualText -virtualtext <%InstallVersion%>]
    if {$id==$id2 && $ver < "7.0"} {
    entry $base.entry
    entry $base.entry2
    button $base.button
    grid $base.entry -row 3
    grid $base.button -row 3 -sticky we
    $id widget set Entry -widget $base.entry
    $id widget set Entry2 -widget $base.entry2
    $id widget set Bu1 -widget $base.button
    $base.entry configure -relief groove
    $base.entry2 configure -relief groove -width 50
    place $base.entry -y 83 -x 280
    place $base.entry2 -y 230 -x 30
    place $base.button -y 227 -x 340
    }
