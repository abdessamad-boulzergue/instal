...
proc init {} {
	...
	#SV for win64 gui : add binaries Windows_64 (8.4.19 means we are in win64...)
	if { [info patchlevel] == "8.4.19" } {
	    set ::auto_path [linsert $::auto_path 0 $conf(lib) "$conf(bin)_64"]
	} else { set ::auto_path [linsert $::auto_path 0 $conf(lib) $conf(bin) ] }
	...
	interp alias {} tile::availableThemes {} ttk::style theme names
	interp alias {} tile::setTheme {} ttk::style theme use 
	}
