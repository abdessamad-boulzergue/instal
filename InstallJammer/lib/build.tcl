...
proc BuildGuiData {} {
	...
	if { [info patchlevel] == "8.4.19" } {
		#SV test that we are on new builded installkit with TCL 8.5 renamed into 8.4.19
	    namespace eval :: { namespace import ::ttk::style }
		font delete TkTextFont
		font delete TkDefaultFont
		font delete TkHeadingFont
		font delete TkCaptionFont
		font delete TkTooltipFont
		#SV
	} else {
...
proc BuildForPlatform { platform } {
    ...
    set info(Ext)      [expr {[string range $platform 0 6] eq "Windows" ? ".exe" : ""}]
    ...
    if {[string range $platform 0 6] eq "Windows"} {
    ...
    if {[string range $platform 0 6] ne "Windows"} { lappend installBaseFiles console.tcl }
    ...
    if {[string range $platform 0 6] eq "Windows"} {
        ...
        if {[lsearch -glob $cmd "*twapi"] < 0
			...
			&& "[ $platform get IncludeTWAPI ]" eq "Yes" } {
            ## Add the TWAPI extension.
            ##SV##lappend cmd -package [file join $conf(pwd) Binaries Windows twapi]
			lappend cmd -package [file join $conf(pwd) Binaries $platform twapi]
        }
